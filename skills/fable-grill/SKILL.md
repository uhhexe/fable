---
name: fable-grill
description: Use when the thinking itself needs facilitation, before anything is built: the user says "grill me", "war-game this", "fable grill", "what am I not seeing", "help me think through this decision or design", has open design questions, or wants ideation structured into decisions. Not for planning a settled build (fable-plan), verifying finished work (fable-review), or debugging a failure.
---
...
**Background:** fable-mode §4 (Facilitation), the sibling skill in this plugin. If it isn't loaded, this file stands alone; the standing moves below carry the essentials.
...
**Terminal state: the ledger on disk plus a handoff pointer** (fable-spec if design remains open, fable-plan if it's build-ready). Never begin building from inside a facilitation session; locked rulings are inputs to the gate, not a go.

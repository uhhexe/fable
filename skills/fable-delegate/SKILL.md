...
Three transports, in descending shared context: **subagent or fresh session** (may share a filesystem, but still write for a cold start), **CLI bridge** (no conversation context, maybe repo access), **paste-only** (no context, no filesystem: the operator is the courier). The brief's self-containment level comes from the transport, so establish it first.
...
1. **Probe the premises before building.** The sender's ground truth may already be stale, so verify the claims that shape the work against live state. A false premise ("build X" when X shipped last week) means surface it, never build on it.
...
- The delegation invisible in the final report, so the operator can't audit what was trusted.
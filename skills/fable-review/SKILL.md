...
3. **Probe each claim live.** Run the tests yourself. Hit the endpoint. Load the page. Re-grep the live file. Merged ≠ shipped ≠ running — verify the surface the claim is actually about. If a load-bearing claim cannot be probed from this environment, say exactly which probe is missing and why it cannot run here; that claim rides no higher than PLAUSIBLE, and the verdict is capped — unprobed load-bearing claims cannot back a clean SHIP.
